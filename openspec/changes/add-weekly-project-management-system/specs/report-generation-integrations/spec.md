...
- **THEN** the system includes project metadata, current plan baseline, weekly planned outcomes, manual weekly updates, current-week newly uploaded or manually entered material summaries, current-week Git commits for the selected tracked branches of connected repositories, GitHub activity summaries, prior current-week report content if any, a fixed system prompt, and the effective report template in the context snapshot
