...
class FrontendTest(unittest.TestCase):
    ...
    def test_todo_board_is_separate_and_closure_collects_required_context(self):
        ...
        self.assertIn('.todo-mode .page-corner-fold { color: var(--board-head-ink);', styles)
        self.assertIn(':root[data-mode="dark"]', styles)
        self.assertIn("function applyAppearance(", source)
        self.assertIn('class="mode-option', source)
        self.assertIn("--board-head-ink: #ffffff;", styles)
        ...
        self.assertIn("btn.dataset.modeTab === state.mode", source)
        ...
        self.assertIn('--board-canvas: color-mix(in srgb, var(--accent) 12%, #12151f);', styles)
