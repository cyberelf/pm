...
class FrontendTest(unittest.TestCase):
    ...
    def test_voice_todo_fab_and_global_agent_setting(self):
        ...
        self.assertIn('id="asr-language-input"', html)
        ...
        self.assertIn('asr_language: $("asr-language-input")?.value || ""', source)
        self.assertIn('state.asrLanguage = data.asr_language || "zh";', source)
        self.assertIn('state.voiceAgent = ["codex", "claude", "internal"].includes(data.voice_agent) ? data.voice_agent : "codex";', source)
        self.assertIn("function renderVoiceSettings()", source)
        self.assertIn('select.value = ["codex", "claude", "internal"].includes(state.voiceAgent) ? state.voiceAgent : "codex";', source)
        self.assertIn('<option value="internal">内部 Agent</option>', html)
