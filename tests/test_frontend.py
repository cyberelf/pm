...
class FrontendTest(unittest.TestCase):
    ...
    def test_voice_todo_fab_and_global_agent_setting(self):
        ...
        self.assertIn('recognition.lang = "zh-CN"', source)
        self.assertIn('"/api/todos/voice"', source)
        self.assertIn('method: "PUT", body: JSON.stringify({ voice_agent: select.value })', source)
        self.assertIn("state.voiceAgent = data.voice_agent === \"claude\" ? \"claude\" : \"codex\";", source)
        self.assertIn("function renderVoiceAgentSetting()", source)
