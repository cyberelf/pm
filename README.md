...
- Users configure their own access tokens in 全局设置 → Git 集成 (`github_token`, `gitlab_token`; stored per account, never returned by the API). Read-only access is enough: a GitHub fine-grained token with `Contents: Read-only` (classic token: `public_repo` or `repo`), and a GitLab personal access token with the `read_api` scope (Reporter role for private projects). Without a token only public repositories are reachable, and GitHub's anonymous rate limit is low. Administrators can disable either integration globally.
