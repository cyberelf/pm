...
- Users configure their own access tokens in 全局设置 → Git 集成 (`github_token`, `gitlab_token`; stored per account, never returned by the API). Without a token only public repositories are reachable, and GitHub's anonymous rate limit is low. Administrators can disable either integration globally.
