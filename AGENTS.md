...
- Stable service: cross-platform login-service scripts in `scripts/` (macOS LaunchAgent, Linux systemd user unit, Windows Startup-folder `.bat` via Git Bash; `uninstall_service.sh [reports|asr]` removes them), plus a Docker Compose deployment mode (`compose.yaml`, `Dockerfile`, `docker/asr/Dockerfile`, `docker/entrypoint.sh`) that also runs the whisper.cpp ASR container, with state in the docker named volume `weekly-reports_reports-data`, separate from `data/`. On NVIDIA GPU hosts layer on `compose.gpu.yaml` (`docker compose -f compose.yaml -f compose.gpu.yaml up -d --build`), which switches the asr build to `docker/asr/Dockerfile.cuda` and reserves all host GPUs.
