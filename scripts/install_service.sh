#!/usr/bin/env bash
# Install the Weekly Reports server as a per-user background service that
# starts at login. One script, three platforms:
#   macOS    LaunchAgent (launchd)
#   Linux    systemd user service (weekly-reports.service)
...
Linux)
  ...
  {
    echo "[Unit]"
    echo "Description=Weekly Reports server"
    echo ""
    echo "[Service]"
...
MINGW*|MSYS*|CYGWIN*)
  ...
  mkdir -p "$STARTUP_DIR"
  BAT="$STARTUP_DIR/weekly-reports.bat"
  {
    echo '@echo off'
  ...
  MSYS2_ARG_CONV_EXCL='*' cmd /c start "weekly-reports" "$BAT_WIN" >/dev/null
  report "$BAT"
