...
全量返回的 `todos` 天然简单：拖拽乐观更新本地状态 → `PUT /api/todos/{id}` → 失败回滚并 toast。
不做合并策略（单人使用，无并发写冲突场景）。下拉刷新 + 切项目时重拉。
...
5. **看板页** — 四状态看板（草稿/待办/进行中/关闭），拖拽换列，深色反色面
...
| M2 看板 | 四栏看板、拖拽流转、关闭/删除、下拉刷新 | 拖动即时反馈且与服务端一致 |
